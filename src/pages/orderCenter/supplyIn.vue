	  ...
	  <div class="purch bg-f7 js-eliHeight">
		<div class="purch-admin" v-if="dataList.length>0">
			<div class="purch-admin-item supply-item" v-for="(item,index) in dataList" :key="index">
              ...
              <div class="item-body">
                <router-link :to="'/orderDetail?type=supplyBuy&id='+item.id">
                    <img :src="item.showProduct_img[0]" />
                    <p class="pt35 pb35">{{ item.order_name }} <span class="fr">数量：x{{ item.order_numb }}</span></p>
                </router-link>
              </div>
              ...
              <div class="item-footer action-wrap">
                ...
                <div class="f-right">
                  <router-link v-if="item.isAudit==0&&(item.status=='待发货'||item.status=='待收货'||item.status=='已完成')" 
                      :to="{path:'/orderAppeal',query:{orderType:'supply',userType:'buy',orderSn:item.order_sn,orderId:item.id}}">
    ...
    <div v-transfer-dom>
      ...
      @on-confirm="acceptAction">
        <form class="purch-form">
                ...
                <input ref="pwd" type="text" maxlength="8" v-model.trim="password" style="position: absolute;z-index: -1;opacity: 0;letter-spacing:10;"/>
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.$api.post(
        this.$interface.apiurl.getSupplyInOrder,params,res => {
    ...
    cancelAction(item) {  //取消订单
      ...
      CommomObj.$comfirm("确定要取消订单吗？", function() {
        ...
        _this.$api.post(_this.$interface.apiurl.cancelOrder,params,res => {
