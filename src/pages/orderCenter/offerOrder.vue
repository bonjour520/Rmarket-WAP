<template>
<div class="x-scroll offerOrder">
	<scroller lock-x scrollbar-y v-show="isShow" v-model="scrollerStatus" style="background: #f7f7f7;" ref="scrollerBottom"
	  ...
	  <div class="purch bg-f7 js-eliHeight">
		<div class="purch-admin" v-if="dataList.length>0">
			<div class="purch-admin-item supply-item" v-for="(item,index) in dataList" :key="index">
              ...
              <div class="item-body">
                <router-link :to="'/orderDetail?type=report&id='+item.order_sn" tag="span">
                    <div class="flex1">
                        <h2>{{ item.order_name }}</h2>
                        <p class="rmt10 layout-flex"><span class="cr-pink">供应ID：{{ item.gscID }}</span></p>
                    </div>
                  ...
                  <div>订单总额： <b class="money">￥{{ item.sumOrderPrice }}</b></div>
                    ...
                    <div class="f-right">
                      <router-link v-if="item.isAudit==0&&(item.status=='待发货'||item.status=='待收货'||item.status=='已完成')" 
                        :to="{path:'/orderAppeal',query:{orderType:'purchase',userType:'sell',orderSn:item.order_sn,orderId:item.id}}">
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.$api.post(this.$interface.apiurl.getPriceOrder,params,res => {
    ...
    deliverAction(item) {
      ...
      CommomObj.$comfirm("确定要发货吗？", function() {
        ...
        _this.$api.post(_this.$interface.apiurl.deliverPriceGoods,params,res => {
    ...
    delAction(item, index) {
      ...
      CommomObj.$comfirm("确定要删除吗？", function() {
        ...
        _this.$api.post(_this.$interface.apiurl.delelteOrder,params,res => {
            _this.$vux.loading.hide();
            CommomObj.$tips("删除成功!");
  ...
  watch:{
    '$route' (to, from) {
      if(from.path=='/orderCenter'||from.path=='/myOffer'){
